<template>
  <div id="detail">
    ...
    <div style="text-align: right; margin-top: -10px;" v-if="tweet">
      ...
      <v-btn flat icon v-if="isFaved()" :disabled="loading" @click="delFav"><v-icon color="red">favorite</v-icon> {{ tweet.favs.length }}</v-btn>
      <v-btn flat icon v-else :disabled="loading" @click="setFav"><v-icon>favorite_border</v-icon> {{ tweet.favs.length }}</v-btn>
